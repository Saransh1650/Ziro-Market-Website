---
title: "Crypto tax in India 2026: the 30% and 1% TDS rules"
seoTitle: "Crypto Tax India 2026 | 30% + 1% TDS Rules"
date: "2026-09-22"
datePublished: "2026-07-07"
category: "concept"
type: "explainer"
excerpt: "India taxes crypto gains at a flat 30% plus cess, adds a 1% TDS on sales, and lets you deduct nothing but cost, with no loss set-off."
primaryKeyword: "crypto tax india 2026"
secondaryKeywords: ["crypto tax rate india", "1 percent TDS crypto", "VDA tax 30 percent", "how is bitcoin taxed india", "crypto tax rules 2026"]
eli5: |
  In India, if you make money from crypto like Bitcoin, the government takes a flat 30% of the profit as tax, no matter how small or large your income otherwise is. On top of that, every time you sell, 1% of the sale amount is set aside as an advance tax called TDS. Unlike with shares, you cannot use crypto losses to reduce the tax on your crypto profits.

  For example, if you buy Bitcoin for Rs 1 lakh and sell it for Rs 1.5 lakh, your Rs 50,000 profit is taxed at 30%, so you pay Rs 15,000 plus a small cess. And if another coin lost you Rs 40,000 that year, you still cannot subtract that loss, you are taxed on the full Rs 50,000 gain. That harsh, no-adjustment rule is what makes India's crypto tax one of the strictest anywhere.
tags: ["crypto", "crypto-tax", "bitcoin", "vda", "tds", "india", "tax", "regulation"]
faq:
  - q: "What is the crypto tax rate in India in 2026?"
    a: "Profits from transferring crypto and other Virtual Digital Assets (VDAs) are taxed at a flat 30%, plus applicable surcharge and a 4% cess, under the rules originally introduced as Section 115BBH. This rate applies regardless of your income tax slab or how long you held the asset. No deduction is allowed except the cost of acquisition, and there is no lower rate for long-term holding. This is general information, not tax advice."
  - q: "What is the 1% TDS on crypto?"
    a: "A 1% TDS (Tax Deducted at Source) applies on the sale consideration when you transfer a VDA above a specified threshold, under the rules originally introduced as Section 194S. It is not an extra tax but an advance deduction that is adjusted against your final tax liability. Its main purpose is to create a transaction trail for the tax authorities, and Indian exchanges deduct it automatically on trades."
  - q: "Can I set off crypto losses against other income?"
    a: "No. Losses from one VDA cannot be set off against gains from another VDA, nor against any other income like salary or capital gains, and they cannot be carried forward to future years. This means if you gained on Bitcoin but lost on another coin in the same year, you are still taxed on the full Bitcoin gain. This no-set-off rule is one of the harshest features of India's crypto tax regime. This is general information, not tax advice."
  - q: "How do I report crypto in my income tax return?"
    a: "Crypto gains are reported under 'Schedule VDA' in your income tax return. For FY 2025-26 (AY 2026-27), you use ITR-2 if you report crypto as capital gains, or ITR-3 if you treat it as business income. You must report each transaction, and the 1% TDS already deducted can be claimed against your total tax. This is general information, not tax advice."
  - q: "Did the new Income Tax Act change crypto tax in 2026?"
    a: "The Income Tax Act, 2025, effective from April 1, 2026, keeps the substance of India's crypto tax unchanged: the 30% flat tax and 1% TDS remain. The new Act renumbers the governing sections, explicitly adds 'crypto-asset' to the VDA definition, and introduces a dedicated penalty regime for failures in crypto-asset reporting. So the rates are the same, but reporting and compliance obligations have tightened. This is general information, not tax advice."
---

India did not ban crypto. It did something that, for many traders, stings almost as much: it taxed it harder than almost anywhere on earth. **Crypto gains are taxed at a flat 30% plus cess, a 1% TDS is skimmed off every sale, and you can deduct nothing but your purchase cost, with losses that cannot be set off against anything.** Understanding these rules before you trade is the difference between a clean return and a nasty reckoning at filing time.

The framework arrived in 2022 and has only tightened since, with the new Income Tax Act, 2025 carrying it forward from April 1, 2026 largely intact.

<Callout type="note" title="Where this stands, 22 September 2026">

Budget 2026 left the 30% tax, the 1% TDS and the ban on loss set-off untouched, but from 1 April 2026 it put penalties on the platforms that must report crypto transactions to the tax department: Rs 200 a day for failing to file, and a flat Rs 50,000 for inaccurate information that is not corrected. With bitcoin touching an eight-month high near $85,000 on 21 September, more Indian traders are booking gains that this regime will tax in full.

</Callout>

![Crypto tax in India 2026: 30% flat tax on gains, 1% TDS on every sale, no loss set-off, reported under Schedule VDA](/images/crypto-tax-2026.svg)

## The three rules that define it

Almost everything about Indian crypto tax flows from three hard rules. **First, a flat 30% rate on any gain from transferring a Virtual Digital Asset**, with no slab benefit and no lower long-term rate, plus surcharge and a 4% cess on top. Whether you held Bitcoin for a day or three years, the rate is the same.

Second, a 1% TDS on the sale value above a threshold, deducted automatically by Indian exchanges. It is not an extra tax but an advance that adjusts against your final bill, designed mainly to give the tax department a trail of every transaction.

Third, and most painful, is the treatment of losses. Here is how it plays out.

| Scenario | Taxable amount |
| --- | --- |
| Rs 50,000 gain on Bitcoin | Full Rs 50,000 taxed at 30% |
| Rs 50,000 gain, Rs 40,000 loss on another coin | Still full Rs 50,000 taxed; loss ignored |
| Rs 30,000 net loss for the year | Nothing to carry forward to next year |

**You are taxed on your winners and get no relief for your losers**, because VDA losses cannot be set off against other VDA gains, against other income, or carried forward.

Here is one trade worked through, using round numbers close to September 2026 prices.

| Step | Amount |
| --- | --- |
| Buy 0.01 bitcoin | Rs 70,000 |
| Sell it on an Indian exchange | Rs 81,000 |
| 1% TDS deducted on the sale value | Rs 810, credited against your final tax |
| Taxable gain (sale minus cost only) | Rs 11,000 |
| Tax at 30% plus 4% cess | Rs 3,432 |
| Still payable after the TDS credit | Rs 2,622 |

TDS applies once your crypto sales cross Rs 50,000 in a financial year for most individuals, or Rs 10,000 for others. **Exchange fees, gas fees and the interest on any money borrowed to buy crypto are not deductible**, which is why the effective rate on active trading ends up higher than the 31.2% headline.

<Logo src="/images/photos/north-block-delhi.jpg" alt="North Block, New Delhi, home of the Finance Ministry that introduced India's 30% crypto tax and 1% TDS in 2022 and kept both in Budget 2026" width={680} caption="North Block, New Delhi. The Finance Ministry introduced the 30% tax in Budget 2022 and the 1% TDS from July 2022; neither has changed since." credit="Photo: Pinakpani / Wikimedia Commons, CC BY-SA 4.0" />

## Why it is called the harshest regime

Compare this with equity. A stock investor gets a lower long-term rate, a Rs 1.25 lakh annual exemption, and the right to offset losses against gains, as covered in our [capital gains tax 2026 guide](/blog/capital-gains-tax-stocks-mutual-funds-2026). **A crypto investor gets none of those cushions**, which is why the effective burden on active crypto trading in India is unusually heavy.

The 1% TDS adds a second layer of friction. For high-frequency traders, having 1% shaved off the value of every sale ties up capital and eats into the compounding that active trading relies on, even though it is eventually adjusted. It is a deliberate drag, and it has visibly pushed volumes toward less-taxed venues.

## What changed for 2026

The rates did not move, but the rulebook did. **The Income Tax Act, 2025, effective April 1, 2026, renumbers the old sections, explicitly folds "crypto-asset" into the VDA definition, and adds a dedicated penalty regime for reporting failures.** In practice that means the tax you pay is the same, but the consequences of not reporting correctly are sharper.

Reporting itself runs through Schedule VDA in the income tax return, with ITR-2 for those declaring capital gains and ITR-3 for those treating crypto as a business. Every transaction has to be disclosed, and the TDS already deducted is claimed back there. For the wider policy picture, including SEBI and RBI's stance, see our [India crypto regulation 2026](/blog/india-crypto-regulation-2026-tax-sebi-rbi) explainer, and for live prices our [bitcoin price today](/blog/bitcoin-price-today) page.

None of this makes crypto illegal, and plenty of Indians still trade it. But the tax code has made one thing unmistakable: in India, crypto is taxed as if the government would rather you thought twice, and the smart move is to price that 30%, that 1%, and that missing loss set-off into every decision before you make it.
